...
transformed parameters {
  ...
  // ランダムウォークするgを変換することで相関係数の-1 - 1に押し込める
  // 制約があるパラメータの収束の頻出テクニック
  // 変換にはFisher's Z変換を使う
  for (t in 1:n) {
...
model {
  ...
  g[1] ~ normal(0, 10); // ランダムウォーク

  // 状態方程式
  for (t in 2:n) {
    for (i in 1:p) {
      h[i, t] ~ normal(
        mu[i] + phi[i] * (h[i, t-1] - mu[i]), sigma_eta[i]
      );
    }
    g[t] ~ normal(g[t-1], sigma_zeta); // ランダムウォーク
  }
